...
export async function registerRoutes(app: Express): Promise<Server> {
  // Add middleware to extract and validate deviceId from headers
  app.use(async (req: Request, res: Response, next) => {
    try {
      // Device ID is required for all requests
      const deviceId = req.headers['x-device-id'] as string;
      if (!deviceId) {
        return res.status(400).json({ error: 'Device ID is required' });
      }

      // Store the device ID in the request object
      (req as any).deviceId = deviceId;
      
      // Get or create user session
      let userId = await storage.getUserIdFromDeviceSession(deviceId);
      if (userId === undefined) {
        userId = 0; // Demo user
        await storage.registerDeviceSession(deviceId, userId);
      }
  ...
  app.get("/api/user/profile", async (req: Request, res: Response) => {
    try {
      // Get the user ID from the request object (set by middleware)
      const userId = (req as any).userId;
      
      // Log the request for debugging
      console.log(`Fetching profile for user ${userId} from device ${(req as any).deviceId}`);
      ...
      res.json({
        ...
        hasSurveyCompleted,
        userProfile,
        deviceId: (req as any).deviceId // Include the device ID in the response
      });
    } catch (error) {
  ...
  app.post('/api/chat/message', async (req: Request, res: Response) => {
    try {
      ...
      // Get current history
      let history = await storage.getChatHistory(
        (req as any).userId,
        (req as any).deviceId
      );

      // Update history with new message
      const updatedHistory = await storage.updateChatHistory(
        (req as any).userId,
        { 
          messages: [
            ...(history?.messages || []),
