...
export interface IStorage {
  ...
  getChatHistory(userId: number, deviceId: string): Promise<ChatHistory | undefined>;
  createChatHistory(chatHistory: InsertChatHistory, deviceId: string): Promise<ChatHistory>;
  updateChatHistory(userId: number, chatHistory: Partial<ChatHistory>, deviceId: string): Promise<ChatHistory>;
}
...
export class MemStorage implements IStorage {
  ...
  private chatHistoryMap: Map<string, ChatHistory>; // key format: "userId:deviceId:recordId"
  ...
  async getChatHistory(userId: number, deviceId: string): Promise<ChatHistory | undefined> {
    // Find the chat history by iterating through entries with matching userId and deviceId
  ...
  async createChatHistory(chatHistory: InsertChatHistory, deviceId: string): Promise<ChatHistory> {
    ...
    // Always use composite key with userId and deviceId
    const key = `${chatHistory.userId}:${deviceId}:${id}`;
    this.chatHistoryMap.set(key, newHistory);
    return newHistory;
  }
  ...
  async updateChatHistory(userId: number, updatedHistory: Partial<ChatHistory>, deviceId: string): Promise<ChatHistory> {
    ...
    if (history) {
      ...
      const key = `${userId}:${deviceId}:${history.id}`;
