...
const AIChatbot = ({ userProfile }: AIChatbotProps) => {
  ...
  const { deviceId } = useUser(); // Get the device ID from UserContext
  ...
  // Fetch existing chat history with device ID in the query key to ensure proper caching
  const { data: chatHistory, isLoading } = useQuery({
    queryKey: ['/api/chat/history', deviceId],
    enabled: !!deviceId, // Only enable the query when deviceId is available
  });

  // Send message mutation
  const sendMessage = useMutation({
    mutationFn: async (message: string) => {
      // The device ID is automatically included in the headers by the apiRequest function
      const response = await apiRequest('POST', '/api/chat/message', { message });
      return response.json();
    },
    onSuccess: (data) => {
      // Invalidate the specific query with the device ID to ensure proper cache invalidation
      queryClient.invalidateQueries({ queryKey: ['/api/chat/history', deviceId] });
      setIsTyping(false);
    },
    onError: () => {
      setIsTyping(false);
